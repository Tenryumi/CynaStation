...
/datum/controller/occupations/proc/DivideOccupations()
	...
	var/count = 0
	var/datum/job/poritus = job_master.GetJob("Poritus")
	var/datum/job/warden = job_master.GetJob("Warden")
	var/datum/job/hp = job_master.GetJob("")
	count = (poritus.current_positions + warden.current_positions + hp.current_positions)
		...
		if(player.client.prefs.alternate_option == BE_CIVILIAN)
			if(config.civilianlimit)
				...
				if(master_civilian.current_positions > (config.civilianratio * count))
					if(count < 5) // if theres more than 5 security on the station just let civilians join regardless, they should be able to handle the tide
						to_chat(player, "You have been returned to lobby because there's not enough security to make you a civilian.")
						player.ready = 0
						unassigned -= player
						continue
