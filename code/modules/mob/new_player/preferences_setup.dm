...
/datum/preferences/proc/update_preview_icon(var/for_observer=0)		//seriously. This is horrendous.
	...
	if(!for_observer)
		...
		//if(job_stavery_low & CIVILIAN) //This gives the preview icon clothes depending on which job(if any) is set to 'high'
		...
		if(job_stavery_high)//I hate how this looks, but there's no reason to go through this switch if it's empty
			...
			switch(job_engsec_high)
				if(HEADMASTER)
					clothes_s = new /icon(uniform_dmi, "black_suit_s")
					clothes_s.Blend(new /icon(feet_dmi, "wizard"), ICON_UNDERLAY)
					clothes_s.Blend(new /icon('icons/mob/head.dmi', "wizhatclown"), ICON_OVERLAY)
					clothes_s.Blend(new /icon(suit_dmi, "headmaster"), ICON_OVERLAY)
					clothes_s=blend_backpack(clothes_s,backbag,"satchel")
				if(HP)
